#include "ValidityCheckers/state_validity_checker_pcc_blackmore_euclidean.hpp"

StateValidityCheckerPCCBlackmoreEuclidean::StateValidityCheckerPCCBlackmoreEuclidean(const std::string &scene_id, const ob::SpaceInformationPtr &si, const double p_safe) :
...
StateValidityCheckerPCCBlackmoreEuclidean::~StateValidityCheckerPCCBlackmoreEuclidean() {
}

bool StateValidityCheckerPCCBlackmoreEuclidean::isValid(const ob::State *state) const {
    ...
    const double x = state->as<R2BeliefSpaceEuclidean::StateType>()->getX();
    const double y = state->as<R2BeliefSpaceEuclidean::StateType>()->getY();
	...
	x_pose = state->as<R2BeliefSpaceEuclidean::StateType>()->getX();
	y_pose = state->as<R2BeliefSpaceEuclidean::StateType>()->getY();
	z_pose = 4.0;
...
bool StateValidityCheckerPCCBlackmoreEuclidean::HyperplaneCCValidityChecker(const Eigen::MatrixXf &A, const Eigen::MatrixXf &B, const double &x_pose, const double &y_pose, const double &z_pose, const Eigen::MatrixXf &PX) const {
