...
bool BarrierTrajectoryValidityChecker::segmentHitsObstacle(
    const Eigen::Vector2d &p1,
    const Eigen::Vector2d &p2) const
{
    for (const auto &box : obstacleAABBs_) {
        double fx = box[0], tx = box[1];
        double fy = box[2], ty = box[3];

        // Check if either endpoint is inside
        if ((p1(0) >= fx && p1(0) <= tx && p1(1) >= fy && p1(1) <= ty) ||
            (p2(0) >= fx && p2(0) <= tx && p2(1) >= fy && p2(1) <= ty)) {
            return true;
        }

        // Check if the segment intersects rectangle edges
        Eigen::Vector2d q1(fx, fy), q2(tx, fy); // bottom
        Eigen::Vector2d q3(tx, ty), q4(fx, ty); // top
        if (segmentsIntersect(p1,p2,q1,q2) ||
            segmentsIntersect(p1,p2,q2,q3) ||
            segmentsIntersect(p1,p2,q3,q4) ||
            segmentsIntersect(p1,p2,q4,q1)) {
            return true;
        }
    }
    return false;
...
bool BarrierTrajectoryValidityChecker::segmentsIntersect(
    const Eigen::Vector2d &p1, const Eigen::Vector2d &p2,
    const Eigen::Vector2d &q1, const Eigen::Vector2d &q2) const
{
    auto orient = [](const Eigen::Vector2d &a, const Eigen::Vector2d &b, const Eigen::Vector2d &c) {
